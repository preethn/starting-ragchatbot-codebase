...
class TestNoToolCalls:
    def test_single_api_call_when_no_tool_use(self, generator, tools):
        ...
        with patch.object(generator.client.messages, "create", return_value=text_response) as mock_create:
    ...
    def test_no_tool_executed_when_no_tool_use(self, generator, tool_manager, tools):
        with patch.object(generator.client.messages, "create", return_value=make_text_response()):
            generator.generate_response("What is ML?", tools=tools, tool_manager=tool_manager)

        tool_manager.execute_tool.assert_not_called()

    def test_works_without_tools(self, generator):
        with patch.object(generator.client.messages, "create", return_value=make_text_response("Hello")) as mock_create:
...
class TestOneToolRound:
    def test_two_api_calls_for_one_tool_round(self, generator, tool_manager, tools):
        responses = [make_tool_use_response(), make_text_response("Answer after search")]
        with patch.object(generator.client.messages, "create", side_effect=responses) as mock_create:
            result = generator.generate_response("Find course X", tools=tools, tool_manager=tool_manager)

        assert mock_create.call_count == 2
        assert result == "Answer after search"

    def test_tool_executed_once(self, generator, tool_manager, tools):
        responses = [make_tool_use_response(tool_input={"query": "course X"}), make_text_response()]
        with patch.object(generator.client.messages, "create", side_effect=responses):
            generator.generate_response("Find course X", tools=tools, tool_manager=tool_manager)

        tool_manager.execute_tool.assert_called_once_with("search_course_content", query="course X")

    def test_second_call_has_tools_when_more_rounds_possible(self, generator, tool_manager, tools):
    ...
    def test_conversation_history_included(self, generator, tool_manager, tools):
        responses = [make_tool_use_response(), make_text_response()]
        with patch.object(generator.client.messages, "create", side_effect=responses) as mock_create:
            generator.generate_response("Find course X", conversation_history="User: hi\nAssistant: hello", tools=tools, tool_manager=tool_manager)
...
class TestTwoToolRounds:
    def test_three_api_calls_for_two_tool_rounds(self, generator, tool_manager, tools):
        ...
        with patch.object(generator.client.messages, "create", side_effect=responses) as mock_create:
            result = generator.generate_response("Compare courses", tools=tools, tool_manager=tool_manager)
    ...
    def test_tool_executed_twice(self, generator, tool_manager, tools):
        ...
        with patch.object(generator.client.messages, "create", side_effect=responses):
            generator.generate_response("Compare courses", tools=tools, tool_manager=tool_manager)

        assert tool_manager.execute_tool.call_count == 2

    def test_tools_stripped_on_final_call_after_two_rounds(self, generator, tool_manager, tools):
    ...
    def test_caps_at_two_rounds_even_if_claude_wants_more(self, generator, tool_manager, tools):
        """Claude signals tool_use three times but the system must stop at 2 rounds."""
        responses = [
            make_tool_use_response(tool_id="t1"),
            make_tool_use_response(tool_id="t2"),
            make_tool_use_response(tool_id="t3"),  # Would trigger a third round — must be ignored
            make_text_response(),
        ]
        with patch.object(generator.client.messages, "create", side_effect=responses) as mock_create:
            ...
            generator.generate_response("Multi search", tools=tools, tool_manager=tool_manager)
...
class TestErrorHandling:
    def test_tool_exception_aborts_loop_gracefully(self, generator, tool_manager, tools):
        ...
        with patch.object(generator.client.messages, "create", side_effect=[tool_use_resp, fallback_text_resp]) as mock_create:
            result = generator.generate_response("Find course X", tools=tools, tool_manager=tool_manager)
    ...
    def test_no_tool_manager_skips_loop(self, generator, tools):
        ...
        with patch.object(generator.client.messages, "create", return_value=tool_use_resp) as mock_create:
            result = generator.generate_response("Find course X", tools=tools, tool_manager=None)
